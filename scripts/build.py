...
KASSA_CFG = {
    ...
    5:  ('km_pos',                       False, False, 'mspos-t',        '',          False),  # только заявка
...
# 9  — PayTor Jay Pro
# 10 — EdPOS
# 11-15 — готовые кассовые комплекты
# 18-19 — ФН+ОФД комплекты
MANAGER_ONLY = {5, 9, 10, 11, 12, 13, 14, 15, 18, 19}

# ──────────────────────────────────────────
# Форма заявки Контура (forms.kontur.ru)
# ──────────────────────────────────────────
APPLICATION_FORM_SCRIPT_SRC = 'https://forms.kontur.ru/widget/loader'
APPLICATION_FORM_WIDGET_ID  = '49ec006a-7a4f-4c54-8ae5-bc70448c2259'

def application_form_modal():
    """Модалка с формой заявки Контура — открывается по openApplicationForm()."""
    return f'''
<script src="{APPLICATION_FORM_SCRIPT_SRC}" async defer></script>
<div id="kform-modal" class="kform-modal" aria-hidden="true">
  <div class="kform-overlay" data-kform-close></div>
  <div class="kform-box" role="dialog" aria-modal="true" aria-labelledby="kform-title">
    <button type="button" class="kform-close" data-kform-close aria-label="Закрыть">&times;</button>
    <h3 id="kform-title" class="kform-title">Оставить заявку</h3>
    <p class="kform-sub">Это оборудование продаётся только через менеджера — заполните форму, и мы свяжемся с вами.</p>
    <div data-form-widget="{APPLICATION_FORM_WIDGET_ID}"></div>
  </div>
</div>
<script>
(function(){{
  var modal=document.getElementById('kform-modal');
  if(!modal)return;
  window.openApplicationForm=function(){{
    modal.classList.add('kform-open');
    modal.setAttribute('aria-hidden','false');
    document.body.style.overflow='hidden';
  }};
  window.closeApplicationForm=function(){{
    modal.classList.remove('kform-open');
    modal.setAttribute('aria-hidden','true');
    document.body.style.overflow='';
  }};
  document.addEventListener('click',function(e){{
    if(e.target.closest('[data-kform-close]'))window.closeApplicationForm();
  }});
  document.addEventListener('keydown',function(e){{
    if(e.key==='Escape')window.closeApplicationForm();
  }});
}})();
</script>'''
...
body{font-family:'Montserrat','Helvetica Neue',Arial,sans-serif;background:#fff;color:var(--black);-webkit-font-smoothing:antialiased}
...
.phone-link{color:#fff;transition:.15s}
.phone-link:hover{color:var(--orange)}
.hdr-burger{display:none;width:36px;height:36px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:transparent;color:#fff;align-items:center;justify-content:center;flex-shrink:0}
.hdr-nav-mob{display:none;position:fixed;top:60px;left:0;right:0;background:var(--black);border-bottom:1px solid rgba(255,255,255,.08);flex-direction:column;padding:8px 16px 16px;z-index:9998}
.hdr-nav-mob.open{display:flex}
.hdr-nav-mob a{color:rgba(255,255,255,.75);font-size:15px;padding:12px 4px;border-bottom:1px solid rgba(255,255,255,.06)}
.hdr-nav-mob .hdr-phone-mob{padding:14px 4px 0;font-size:15px;font-weight:700;color:#fff}

/* FORM MODAL (заявка) */
.kform-modal{position:fixed;inset:0;z-index:99999;display:none}
.kform-modal.kform-open{display:block}
.kform-overlay{position:absolute;inset:0;background:rgba(15,15,20,.6)}
.kform-box{position:relative;z-index:1;max-width:480px;width:90%;margin:8vh auto 0;background:#fff;border-radius:18px;padding:28px;box-shadow:0 20px 60px rgba(0,0,0,.25);max-height:82vh;overflow-y:auto}
.kform-close{position:absolute;top:12px;right:16px;background:none;border:none;font-size:26px;line-height:1;cursor:pointer;color:var(--muted)}
.kform-title{font-size:20px;font-weight:700;margin-bottom:8px}
.kform-sub{font-size:13px;color:var(--muted);line-height:1.5;margin-bottom:18px}
...
@media(max-width:768px){
  ...
  .footer-in{grid-template-columns:1fr;padding:32px 16px;gap:28px}
  .footer-bot{flex-direction:column;gap:8px;padding:16px;text-align:center}
  .kform-box{width:94%;padding:22px;margin-top:6vh}
}
@media(max-width:420px){
  .kform-title{font-size:18px}
}
</style>'''
...
(function(){
  var burger=document.getElementById('hdr-burger'),nav=document.getElementById('hdr-nav-mob');
  if(burger&&nav){
...
<header class="hdr">
  <div class="hdr-in">
    ...
    <div class="hdr-right">
      <div class="hdr-phone"><a href="tel:88005002244" class="phone-link">8 800 500-22-44</a><small>Бесплатно по России</small></div>
      ...
      <button class="hdr-burger" id="hdr-burger" aria-label="Меню">
  ...
  <nav class="hdr-nav-mob" id="hdr-nav-mob">
    <a href="/kontur-feed/#kasses">Кассы</a>
    <a href="/kontur-feed/#kits">Комплекты</a>
    <a href="/kontur-feed/#fn-ofd">ФН и ОФД</a>
    <a href="/kontur-feed/#edo">КЭП и ЭДО</a>
    <a href="/kontur-feed/#periphery">Периферия</a>
    <a href="/kontur-feed/cart/">Корзина</a>
    <div class="hdr-phone-mob"><a href="tel:88005002244" class="phone-link">8 800 500-22-44</a></div>
  </nav>
</header>
...
<footer class="footer">
  <div class="footer-in">
    <div>
      ...
      <address class="footer-addr"><strong><a href="tel:88005002244" class="phone-link">8 800 500-22-44</a></strong>Бесплатно по России, 24/7</address>
    </div>
  ...
  <div class="pcard-body">
    ...
    def kcard(p):
        ...
        pr_html  = (('<span class="old-price">' + fmt(p['oldPrice']) + '</span> ')
                    if p['oldPrice'] > 0 else '')
        if is_mgr:
            btn = '<button type="button" class="btn-kit btn-kit-mgr" onclick="event.stopPropagation();openApplicationForm()">Оставить заявку</button>'
        else:
...
def build_product(p, all_products, updated):
  ...
  <div class="cfg-tot">
    ...
    if manager_only:
        # Только заявка — открывает форму Контура вместо покупки
        buy_click  = "openApplicationForm()"
...
function cfgBuy(){
  if(!ONLINE_SALE){
    /* Товары только по заявке — открываем форму Контура */
    if(window.openApplicationForm){window.openApplicationForm()}else{window.location.href='tel:88005002244'}
    return;
  }
  var c=getCart();
...
<div class="kp-wrap">
  ...
  <div class="kp-grid">
    ...
    <div>
      ...
      <button class="btn-consult" onclick="openApplicationForm()">Получить консультацию</button>
      {kstore_btn}
      <div class="kp-meta">
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(btn)btn.addEventListener('click',function(){
    ...
    else if(window.openApplicationForm){window.openApplicationForm()}
